...
**Integer label maps** (segmentations) are colored per label rather than shown as grayscale. Controlled by the `SegmentationColors` key in `MIQ.settings.ini`:

- **auto** (default): detected label volumes are colored. **FreeSurfer** segmentations (`aseg`/`aparc`) use their canonical colors; other label maps get distinct per-label colors. A binary mask (single label) is shown white.
- **random**: as auto, but always uses per-label colors (never the FreeSurfer palette).
- **off**: grayscale for every file.

Detection is conservative, so ordinary intensity images stay grayscale.

> [!NOTE]
> `auto` is the default for **new installations only**. If you are upgrading and already have a `MIQ.settings.ini`, it keeps your existing setting — set `SegmentationColors = auto` yourself to enable coloring.
...
MIQ-Win bundles third-party components ([libdeflate](https://github.com/ebiggers/libdeflate) and .NET Framework BCL backports, all MIT-licensed) and builds against the MIT-licensed [QuickLook.Common](https://github.com/QL-Win/QuickLook) host API. Their copyright and permission notices are reproduced in [THIRD-PARTY-NOTICES.md](./THIRD-PARTY-NOTICES.md), which is included in every release package.
